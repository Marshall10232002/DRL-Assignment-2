...
# --- TD Learning for Stage 3 ---
# Stage 3 uses samples collected from Stage 2.
def td_learning_stage3(env, approximator, samples, num_episodes=30000, alpha=0.1, gamma=1):
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % 100 == 0:
            ...
            print(f"Stage 3 - Episode {episode+1} | Time: {elapsed:.2f}s | Avg Reward (last 100): {avg_reward:.2f} | Max Reward (last 100): {max_reward}")
    return approximator.weights

def main():
    with open("stage2_samples.pkl", "rb") as f:
        stage2_samples = pickle.load(f)
    env = Game2048Env()
    patterns = [
        [(0,0), (1,0), (2,0), (3,0)],
        [(1,0), (1,1), (1,2), (1,3)],
    ...
    final_weights = td_learning_stage3(env, approximator, stage2_samples, num_episodes=10000, alpha=0.1, gamma=1)
    with open("stage3_weights.pkl", "wb") as f:
        pickle.dump(final_weights, f)
    print("Stage 3 training complete. Weights saved.")
