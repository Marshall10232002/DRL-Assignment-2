...
import copy
import time
from student_agent import Game2048Env  # Import the environment definition
...
# --- TD Learning for Stage 1 ---
# The splitting condition is met when a board reaches a 16384-tile.
# sample_limit defines the maximum number of board samples to collect.
def td_learning_stage1(env, approximator, num_episodes=10000, alpha=0.1, gamma=1, sample_limit=100):
    samples = []  # collected samples when 16384-tile is reached
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            if not legal_moves:
                break
            # Pure greedy action selection: choose the move with highest estimated value.
            best_val = -float('inf')
            best_action = None
        ...
        if (episode + 1) % 100 == 0:
            ...
            print(f"Stage 1 - Episode {episode+1} | Time: {elapsed:.2f}s | Avg Reward (last 100): {avg_reward:.2f} | Max Reward (last 100): {max_reward}")
        # Save sample if splitting condition is met and sample limit not reached.
        if len(samples) < sample_limit and np.max(env.board) >= 16384:
            samples.append((env.board.copy(), env.score))
    return samples

def main():
    env = Game2048Env()
    # Define your n-tuple patterns (adjust as needed)
    patterns = [
        [(0,0), (1,0), (2,0), (3,0)],
        [(1,0), (1,1), (1,2), (1,3)],
        [(0,0), (0,1), (0,1), (1,1)],
    ...
    samples = td_learning_stage1(env, approximator, num_episodes=30000, alpha=0.1, gamma=1, sample_limit=100)
    with open("stage1_weights.pkl", "wb") as f:
        pickle.dump(approximator.weights, f)
    with open("stage1_samples.pkl", "wb") as f:
        pickle.dump(samples, f)
    print("Stage 1 training complete. Weights and samples saved.")
